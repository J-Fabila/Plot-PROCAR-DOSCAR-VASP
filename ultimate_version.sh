...
#cat atomo${numero}_orbital_${orbital}_up.dat | tr '.' ','| grep -w -- "$(echo $en | tr '.' ',' )" | awk '{print $2}' | tr ',' '.' >> sum_aux_up
            fgrep -wF -- "$energia" atomo${numero}_orbital_${orbital}_up.dat | awk '{print $2}' >> sum_aux_up
            fgrep -wF -- "$energia" atomo${numero}_orbital_${orbital}_down.dat | awk '{print $2}' >> sum_aux_down
#            grep -w -- "$energia" atomo${numero}_orbital_${orbital}_down.dat | awk '{print $2}' >> sum_aux_down
         done
         total_up=$(awk 'BEGIN {FS="\n"} ; {sum+=$1} END {print sum}' sum_aux_up)
         total_down=$(awk 'BEGIN {FS="\n"} ; {sum+=$1} END {print sum}' sum_aux_down)
         if [ $(echo $total_up | wc -c ) -gt 1 ]
         then
            echo $energia $total_up     >> group_${orbital}_${initial_atom}-${final_atom}_up.dat
         fi
         if [ $(echo $total_down | wc -c ) -gt 1 ]
         then
